...
`define JALOP_OFFSET		`MEMAS_END_OFFSET+1	// If something is jump and link

`define JROP_OFFSET		`JALOP_OFFSET+1		// If something is a jump with register

`define CONTROL_BITS_END	`JROP_OFFSET		// The last index in the control bit vector

`define NOP_CONTROL_BITS	18'b00000000000000000	// Control bits for a NOP
 ...
 *
 *		JRop		0 <- not a JR (if still a jump, use normal jump bits from instr)
 * 				1 <- JR (set jump addr to Rs, which is inputA)
 */